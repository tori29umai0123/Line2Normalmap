...
def install_extension_from_url(dirname, url, branch_name=None):
    ...
    try:
        ...
        # pygit2を使用してリポジトリをクローン
        repo = pygit2.clone_repository(url, tmpdir)
        if branch_name:
            # 特定のブランチをチェックアウト
            reference_name = f'refs/remotes/origin/{branch_name}'
            repo.checkout(reference_name)
            # ブランチを指定した場合、そのブランチに切り替える
            repo.reset(repo.lookup_branch(branch_name).target, pygit2.GIT_RESET_HARD)
